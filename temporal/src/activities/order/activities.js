...
const orderActivities = {
  ...
  async validateOrder({ orderId, items, customerId }) {
    ...
    // Validate items
    const invalidItems = items.filter(item => !item.productId || !item.quantity || item.quantity <= 0);
    ...
    console.log(`[OrderActivity] Order ${orderId} validated: $${subtotal.toFixed(2)}`);
  ...
  async checkInventory({ items }) {
    ...
    const outOfStock = inventoryResults.filter(r => r.available < r.requested);
    
    if (outOfStock.length > 0) {
      console.log(`[OrderActivity] Inventory check failed for ${outOfStock.length} items`);
      return {
        success: false,
  ...
  async processPayment({ orderId, amount, paymentMethod }) {
    console.log(`[OrderActivity] Processing payment for order ${orderId}: $${amount}`);
  ...
  async fulfillOrder({ orderId, items }) {
    ...
    console.log(`[OrderActivity] Order ${orderId} fulfilled with ${shipments.length} shipments`);
...
module.exports = orderActivities;
